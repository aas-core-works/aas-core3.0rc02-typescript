...
function constructMatchesXsDateTimeStampUtc(): RegExp {
  const digit = "[0-9]";
  ...
  const timezoneFrag = "Z";
  const dateTimeStampLexicalRep = `${yearFrag}-${monthFrag}-${dayFrag}T((${hourFrag}:${minuteFrag}:${secondFrag})|${endOfDayFrag})${timezoneFrag}`;
  const pattern = `^${dateTimeStampLexicalRep}$`;

  return new RegExp(pattern, "u");
}

const REGEXP_MATCHES_XS_DATE_TIME_STAMP_UTC = constructMatchesXsDateTimeStampUtc();
 ...
 */
export function matchesXsDateTimeStampUtc(text: string): boolean {
  return REGEXP_MATCHES_XS_DATE_TIME_STAMP_UTC.test(text);
}
 ...
 */
export function isXsDateTimeStampUtc(value: string): boolean {
...
function constructMatchesXsDateTimeStamp(): RegExp {
  const digit = "[0-9]";
  const yearFrag = `-?(([1-9]${digit}${digit}${digit}+)|(0${digit}${digit}${digit}))`;
  const monthFrag = "((0[1-9])|(1[0-2]))";
  const dayFrag = `((0[1-9])|([12]${digit})|(3[01]))`;
...
const REGEXP_MATCHES_XS_DATE_TIME_STAMP = constructMatchesXsDateTimeStamp();
 ...
 * @returns True if the `text` conforms to the pattern
 */
export function matchesXsDateTimeStamp(text: string): boolean {
  return REGEXP_MATCHES_XS_DATE_TIME_STAMP.test(text);
}
...
function constructMatchesXsDayTimeDuration(): RegExp {
  const dayTimeDurationRep =
    "-?P((([0-9]+D)(T(([0-9]+H)([0-9]+M)?([0-9]+(\\.[0-9]+)?S)?|([0-9]+M)([0-9]+(\\.[0-9]+)?S)?|([0-9]+(\\.[0-9]+)?S)))?)|(T(([0-9]+H)([0-9]+M)?([0-9]+(\\.[0-9]+)?S)?|([0-9]+M)([0-9]+(\\.[0-9]+)?S)?|([0-9]+(\\.[0-9]+)?S))))";
  const pattern = `^${dayTimeDurationRep}$`;

  return new RegExp(pattern, "u");
}

const REGEXP_MATCHES_XS_DAY_TIME_DURATION = constructMatchesXsDayTimeDuration();
 ...
 * @returns True if the `text` conforms to the pattern
 */
export function matchesXsDayTimeDuration(text: string): boolean {
  return REGEXP_MATCHES_XS_DAY_TIME_DURATION.test(text);
}

function constructMatchesXsYearMonthDuration(): RegExp {
  const yearMonthDurationRep = "-?P((([0-9]+Y)([0-9]+M)?)|([0-9]+M))";
  const pattern = `^${yearMonthDurationRep}$`;

  return new RegExp(pattern, "u");
}

const REGEXP_MATCHES_XS_YEAR_MONTH_DURATION = constructMatchesXsYearMonthDuration();
 ...
 * @returns True if the `text` conforms to the pattern
 */
export function matchesXsYearMonthDuration(text: string): boolean {
  return REGEXP_MATCHES_XS_YEAR_MONTH_DURATION.test(text);
}
...
function constructMatchesXsString(): RegExp {
  const pattern = "^[\\u{1}-\\u{d7ff}\\u{e000}-\\u{fffd}\\u{10000}-\\u{10ffff}]*$";
 ...
 *
 * @param value - to be be checked
 * @returns `true` if `value` is a valid `xs:dateTimeStamp`
 */
export function isXsDateTimeStamp(value: string): boolean {
...
>([
  ...
  [AasTypes.DataTypeDefXsd.DateTimeStamp, isXsDateTimeStamp],
  ...
  [AasTypes.DataTypeDefXsd.DayTimeDuration, matchesXsDayTimeDuration],
  [AasTypes.DataTypeDefXsd.YearMonthDuration, matchesXsYearMonthDuration],
...
function constructMatchesGlobalAssetIdLiterally(): RegExp {
  const pattern = "^[gG][lL][oO][bB][aA][lL][aA][sS][sS][eE][tT][iI][dD]$";

  return new RegExp(pattern, "u");
}

const REGEXP_MATCHES_GLOBAL_ASSET_ID_LITERALLY =
  constructMatchesGlobalAssetIdLiterally();
 ...
 *
 * @returns True if the `text` matches case-insensitive
 */
export function matchesGlobalAssetIdLiterally(text: string): boolean {
  return REGEXP_MATCHES_GLOBAL_ASSET_ID_LITERALLY.test(text);
}
...
> {
  *transformExtensionWithContext(
    ...
    if (context === true) {
      ...
      if (that.refersTo !== null) {
        for (const error of this.transformWithContext(that.refersTo, context)) {
  ...
  *transformAssetInformationWithContext(
    ...
    if (context === true) {
      if (that.globalAssetId !== null) {
        for (const error of this.transformWithContext(that.globalAssetId, context)) {
  ...
  *transformSubmodelWithContext(
    ...
    if (
      !(
        ...
        AasCommon.every(
          AasCommon.map(that.submodelElements, (element) => element.idShort !== null)
        )
      )
    ) {
      yield new VerificationError(
    ...
    if (
      !(
        ...
        that.kindOrDefault() == AasTypes.ModelingKind.Template
      )
    ) {
      yield new VerificationError(
  ...
  *transformSubmodelElementCollectionWithContext(
    ...
    if (
      ...
      yield new VerificationError("ID-shorts need to be defined for all the elements.");
    }
  ...
  *transformEntityWithContext(
    ...
    if (
      !(
        (that.entityType == AasTypes.EntityType.SelfManagedEntity &&
          ((that.globalAssetId !== null && that.specificAssetId === null) ||
            (that.globalAssetId === null && that.specificAssetId !== null))) ||
        (that.globalAssetId === null && that.specificAssetId === null)
      )
    ) {
    ...
    if (context === true) {
      ...
      if (that.specificAssetId !== null) {
        for (const error of this.transformWithContext(that.specificAssetId, context)) {
          error.path.prepend(new PropertySegment(that, "specificAssetId"));
          yield error;
        }
      }
    }
  ...
  *transformEventPayloadWithContext(
    ...
    if (!isModelReferenceToReferable(that.source)) {
      yield new VerificationError("Source must be a model reference to a referable.");
    }
  ...
  *transformConceptDescriptionWithContext(
    ...
    if (
      !(
        !(that.category !== null) ||
        AasConstants.VALID_CATEGORIES_FOR_CONCEPT_DESCRIPTION.has(that.category)
      )
    ) {
      yield new VerificationError(
  ...
  *transformReferenceWithContext(
    ...
    if (
      !(
        !(
          that.type == AasTypes.ReferenceTypes.GlobalReference && that.keys.length >= 1
        ) ||
  ...
  *transformDataSpecificationPhysicalUnitWithContext(
    ...
    if (!(that.definition.length >= 1)) {
      yield new VerificationError("Definition must have at least one item");
    }

    if (!langStringsHaveUniqueLanguages(that.definition)) {
      yield new VerificationError("Definition specifies no duplicate languages");
    }
 ...
 * @returns errors, if any
 */
export function* verifyDateTimeStampUtc(
  that: string
): IterableIterator<VerificationError> {
  if (!matchesXsDateTimeStampUtc(that)) {
    yield new VerificationError(
 ...
 * @returns errors, if any
 */
export function* verifyIdShort(that: string): IterableIterator<VerificationError> {
  if (!(that.length <= 128)) {
    yield new VerificationError(
